...
public class SubmitStartFormCmd implements Command<ProcessInstance>, Serializable {
  ...
  public ProcessInstance execute(CommandContext commandContext) {
    ...
    EngineAdapter.addExecution(processInstance);

    ActivityImpl batchActivity = processDefinition.getActivities().stream().filter(activity -> activity.isBlock()).findAny().orElse(null);
    if(batchActivity != null){
      processInstance.setBlock(true);
    }
